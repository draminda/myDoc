...
import com.decans.mydoc.config.KafkaProperties;
...
class MyDocKafkaResourceIT {
    ...
    void setup() {
        ...
        MyDocKafkaResource kafkaResource = new MyDocKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/my-doc-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/my-doc-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
